...
class OrderedListItemConversionReaction
  ...
  void onPrefixMatched(
    ...
    final numberTyped =
        int.parse(match.substring(numberMatch.start, numberMatch.end));
    ...
    if (numberTyped > 1) {
      ...
      int upstreamListItemOrdinalValue =
          computeListItemOrdinalValue(upstreamNode, document);
...
class HorizontalRuleConversionReaction extends EditReaction {
  ...
  void react(EditContext editorContext, RequestDispatcher requestDispatcher,
    ...
    final edit = changeList.reversed.firstWhere((edit) => edit is DocumentEdit)
        as DocumentEdit;
...
/// USERNOTE: ImageUrlConversionReaction is an auto-formatter that runs after the user presses ENTER (SubmitParagraphIntention).
...
// USERNOTE: Why check from paragraph event? Because the point is to transform image url that is as paragraph into image node.
class ImageUrlConversionReaction extends EditReaction {
  ...
  void react(EditContext editContext, RequestDispatcher requestDispatcher,
    ...
    // USERNOTE: Find the preceding `SelectionChangeEvent` to know which node was active before the split.
    ...
    // USERNOTE: Unexpected: We already know user pressed "enter" from a paragraph via SubmitParagraphIntention editEvent,
    // but somehow the previously selected node is not a paragraph.
    ...
    // USERNOTE: We only care about links.
    ...
    // USERNOTE: Grap the first extracted link, and if that link is not the entire content of the target paragraph, it means user have more text after the link so we skip this reaction.
    final url =
    ...
    // USERNOTE: Verify if the link is an image
    ...
    _isImageUrl(url).then((isImage) {
      ...
      // USERNOTE: Since we asynchronously checked if the extracted url is an image, we have to verify the extracted text hasn't changed during this cooperative yield to the event loop; if the user kept typing, the paragraph may no longer be a pure URL and we must abort the replacement to avoid corrupting the document.
...
class LinkifyReaction extends EditReaction {
  ...
  void _extractUpstreamWordAndLinkify(AttributedText text, int endOffset) {
    final wordStartOffset =
        _moveOffsetByWord(text.toPlainText(), endOffset, true) ?? 0;
  ...
  void _tryUpdateLinkAttribution(
      RequestDispatcher requestDispatcher,
      Document document,
    ...
    if (changeList.length == 1) {
      ...
      final lastSelectionEventIndex =
          changeList.lastIndexWhere((change) => change is SelectionChangeEvent);
    ...
    if (updatePolicy == LinkUpdatePolicy.update) {
      final existingLinkAttribution = changedNodeText
          .getAllAttributionsAt(rangeToUpdate.start)
      ...
      if (existingLinkAttribution != null) {
        final newLinkText = changedNodeText
            .toPlainText()
        ...
        if (newLinkUri != null && newScheme != null && newScheme.isNotEmpty) {
          ...
          if (existingLinkAttribution.hasStructuredUri &&
              existingLinkAttribution.uri!.scheme.isNotEmpty) {
            ...
            updatedAttribution =
                LinkAttribution.fromUri(Uri.parse("$scheme://$newLinkText"));
          } else {
...
Uri? tryToParseUrl(String word) {
  ...
  final int emailCount = extractedEmails.fold(
      0, (value, element) => element is EmailElement ? value + 1 : value);
  if (emailCount == 1) {
...
class DashConversionReaction extends EditReaction {
  ...
  void react(EditContext editorContext, RequestDispatcher requestDispatcher,
    ...
    final upstreamCharacter =
        insertionNode.text.toPlainText()[dashInsertionEvent.offset - 1];
      ...
      DeleteContentRequest(
        documentRange: DocumentRange(
          start: DocumentPosition(
              ...
              nodePosition:
                  TextNodePosition(offset: dashInsertionEvent.offset - 1)),
          end: DocumentPosition(
              nodeId: insertionNode.id,
              nodePosition:
                  TextNodePosition(offset: dashInsertionEvent.offset + 1)),
...
class EditInspector {
  ...
  static UserTypedText? findLastTextUserTyped(
    ...
    final lastSpaceInsertion = edits.whereType<DocumentEdit>().lastWhereOrNull(
        (edit) =>
            edit.change is TextInsertionEvent &&
            (edit.change as TextInsertionEvent)
                .text
                .toPlainText()
                .endsWith(" "));
    ...
    final spaceInsertionChangeIndex =
        edits.indexWhere((edit) => edit == lastSpaceInsertion);
    final selectionAfterInsertionIndex = edits.indexWhere(
        (edit) => edit is SelectionChangeEvent, spaceInsertionChangeIndex);
